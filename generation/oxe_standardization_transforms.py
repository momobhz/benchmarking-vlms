...
def uniform_step_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    
    if "natural_language_instruction" in step["observation"]:
        step["language_instruction"] = step["observation"][
            "natural_language_instruction"
        ]
    elif "language_instruction" in step["observation"]:
        step["language_instruction"] = step["observation"][
            "language_instruction"
        ]
        
    if "natural_language_embedding" in step["observation"]:
        step["observation"]["natural_language_instruction"] = None
...
def berkeley_autolab_ur5_dataset_transform(
    ...
    step["observation"]["depth"] = step["observation"].pop(
        "image_with_depth"
    )

    # make gripper action absolute action, +1 = open, 0 = close
    gripper_action = np.array([step["observation"]["robot_state"][13]])
    step["observation"]["gripper_position"] = gripper_action
    
    step = uniform_step_transform(step)
    return step


def rt1_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    # make gripper action absolute action, +1 = open, 0 = close
    gripper_action = step["observation"]["gripper_closed"][0]
    step["observation"]["gripper_position"] = gripper_action
...
def kuka_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["gripper_closed"][0]
    step = uniform_step_transform(step)
    return step


def taco_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["robot_obs"][6]
    step = uniform_step_transform(step)
    return step

def viola_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["gripper_states"][0]
    step = uniform_step_transform(step)
    return step

def stanford_kuka_multimodal_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["state"][-1]
    step = uniform_step_transform(step)
    return step

def austin_buds_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["state"][7]
    step = uniform_step_transform(step)
    return step

def austin_sirius_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["state_gripper"][0]
    step = uniform_step_transform(step)
    return step

def berkeley_mvp_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["gripper"]
    step = uniform_step_transform(step)
    return step

def asu_table_top_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["state"][-1]
    step = uniform_step_transform(step)
    return step

def nyu_rot_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["state"][-1]
    step = uniform_step_transform(step)
    return step


def berkeley_fanuc_dataset_transform(step: Dict[str, Any]) -> Dict[str, Any]:
    step["observation"]["gripper_position"] = step["observation"]["state"][6]
    step = uniform_step_transform(step)
    return step

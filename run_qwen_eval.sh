...
CURATION_RESULTS="${CURATION_RESULTS:-$TEAM_ROOT/outputs/robo2vlm_spatial_affordance/curation_results.jsonl}"
DATASET_NAME="${DATASET_NAME:-keplerccc/Robo2VLM-1}"
DATASET_SPLIT="${DATASET_SPLIT:-test}"
EVAL_SUBSET="${EVAL_SUBSET:-spatial}"
...
if [ ! -f "$CURATION_RESULTS" ]; then
  echo "Curation results file does not exist: $CURATION_RESULTS" >&2
  exit 1
fi
...
python3 "$EVAL_SCRIPT" \
  ...
  --split "$DATASET_SPLIT" \
  --subset "$EVAL_SUBSET" \
  --curation-results "$CURATION_RESULTS" \
